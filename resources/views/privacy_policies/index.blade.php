<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center bg-white shadow-md px-6 py-4 rounded-lg">
            <h2 class="font-bold text-2xl text-gray-800">Privacy Policies</h2>
            @can('add privacy policy')
                ...
                <table class="w-full text-sm text-left border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    @forelse ($policies as $policy)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 font-medium">{{ $policy->heading }}</td>
                            <td class="px-6 py-4">{{ Str::limit(strip_tags($policy->content), 80) }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-3">
                                    @can('edit privacy policy')
                                        <a href="{{ route('privacy-policy.edit', $policy->id) }}"
                                           class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md text-sm font-semibold transition shadow">
                                            ✏️ Edit
                                        </a>
                                    @endcan
                                    @can('delete privacy policy')
                                        <form action="{{ route('privacy-policy.delete', $policy->id) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-6 text-gray-400 text-base">No privacy policies found.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
